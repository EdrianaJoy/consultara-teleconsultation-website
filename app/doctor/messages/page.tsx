 ...
 * 
 * @module app/doctor/messages/page
 */
...
/**
 * Doctor Messages Page Component
 */
export default function DoctorMessagesPage() {
  ...
  const doctorConversations = conversations.filter(
    c => c.doctorId === user?.id
  );
  ...
  const selectedConv = doctorConversations.find(c => c.id === selectedConversation);

  // Filter conversations by search
  const filteredConversations = doctorConversations.filter(conv => {
    const patientName = `Patient ${conv.patientId.slice(-4)}`;
          ...
          {filteredConversations.length === 0 ? (
            <div className="p-4 text-center text-muted-foreground">
              ...
              <p className="text-sm mt-2">
                Patient messages will appear here after appointments.
              </p>
            </div>
          ) : (
            filteredConversations.map((conv) => {
              ...
              const unreadCount = messages.filter(
                m => m.conversationId === conv.id && !m.read && m.senderType === "patient"
              ).length;
              ...
              return (
                ...
                >
                  <div className="w-12 h-12 rounded-full bg-accent overflow-hidden flex-shrink-0 flex items-center justify-center">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-foreground">
                    Patient {selectedConv.patientId.slice(-4)}
                  </p>
